...
- **`process()`** — drain path, called from the platform cron and optionally at the tail of inbound webhook handling. Delivers pending requests one by one in FIFO insertion order; stops at the first transient failure to avoid hammering a struggling API.
...
match ($result) {
    SubmitResult::SentImmediately => null,          // delivered or already cancelled — done
...
echo $result->stoppedOnTransientFailure; // true → API was unreachable, cooldown begun
...
This is the only mandatory platform-specific code. Implement five methods over your platform's durable store.
...
class MyRetryQueueStorage implements RetryQueueStorageInterface
{
    ...
    public function enqueue(QueuedRequest $request): void
    {
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO comfino_request_queue
             (operation_type, payload, attempts, dedup_key, last_error, created_at)
             VALUES (?, ?, ?, ?, ?, ?)'
        );
        $stmt->execute([
    ...
    {
        // Return oldest-first. Use row locking if your platform supports it
        // to prevent concurrent drains from double-sending.
        $stmt = $this->db->prepare(
            'SELECT * FROM comfino_request_queue ORDER BY id ASC LIMIT ?'
        );
        $stmt->execute([$limit]);
    ...
    {
        $stmt = $this->db->prepare(
            'UPDATE comfino_request_queue SET attempts = ?, last_error = ? WHERE id = ?'
        );
        $stmt->execute([$request->attempts, $request->lastError, $request->id]);
    }
    ...
    public function count(): int
    {
        return (int) $this->db->query(
            'SELECT COUNT(*) FROM comfino_request_queue'
        )->fetchColumn();
    }
}
```
...
Dead-lettered requests additionally trigger `DeadLetterReporterInterface::report()` for platform-level alerting.
...
| `QueueErrorDisposition` | `Retry` / `DropPermanent` / `TreatAsSuccess` |
