...
The `Comfino\Backend\Queue` subsystem provides a durable FIFO retry queue for idempotent outbound ComfinoPay API calls. It ships with a `cancel_order` handler and is designed to be extended with additional operation types (e.g., `report_error`).

## Overview

Without the queue, order cancellation calls happen synchronously inside the shop request thread. If the ComfinoPay API is slow or unavailable, the thread blocks for the full client timeout, degrading UX and risking worker exhaustion.
...
- **Delivery with the right credentials.** A handler implementing `TenantAwareRetryableOperationHandlerInterface` is
  given the request's `tenantKey` and can resolve that merchant's API key at delivery time — see
  [Per-tenant credentials on the drain](#per-tenant-credentials-on-the-drain) below.

#### Per-tenant credentials on the drain

The drain does not run in the request that enqueued the work: it runs under the platform's cron/CLI scope, where the
ambient configuration is the *default* store's. A handler holding one API client resolved from that scope therefore
signs every merchant's queued request with the default merchant's key. The API answers 401, the classifier reads that
as permanent, and the request is dropped — the queue loses exactly the traffic it exists to protect, and only for the
tenants that are not the default one. It is not hypothetical: this is what silently lost `cancel_order` deliveries in
Magento 4.0.0/4.0.1.

The payload cannot fix this on its own, because credentials must be read at delivery time, not enqueue time. Instead,
a handler declares that it wants to know who it is delivering for:

```php
use Comfino\Backend\Queue\TenantAwareRetryableOperationHandlerInterface;

final class MyTenantAwareHandler implements TenantAwareRetryableOperationHandlerInterface
{
    public function execute(array $payload): void
    {
        $this->executeForTenant($payload, null); // Ambient scope — a single-shop install.
    }

    public function executeForTenant(array $payload, ?string $tenantKey): void
    {
        $this->clientForTenant($tenantKey)->syncStock($payload['productId'], (int) $payload['quantity']);
    }
}
```

`OutboundRequestQueue` calls `executeForTenant()` whenever a handler implements the interface, on both `submit()` and
`process()`, and falls back to `execute()` otherwise — so a single-shop plugin's handlers need no changes.

The built-in `ReportErrorHandler` implements it. Give it a client *factory* instead of a client, and it resolves the
merchant's credentials per delivery (`ErrorLogger` records its own tenant scope on every report it enqueues):

```php
$tenantClientFactory = static fn (?string $tenantKey): ClientInterface => $sharedClient->bind(
...
$queue = (new OutboundRequestQueueFactory())->create(
    ...
    tenantClientFactory: $tenantClientFactory, // Required in a multi-tenant host
);
```

The factory receives `null` for a request that carries no tenant identity — a single-shop install, or a row enqueued
before the tenant key existed — and should return the ambient scope's client for it, which is what the request was
enqueued with. If it cannot resolve a tenant, it should throw: the queue treats that as transient and retries, so a
report enqueued while a store's configuration was mid-edit is not lost to the drain that raced it.

`cancel_order` is deliberately left on the ambient client even when a factory is passed: its payload is
platform-shaped (Magento keys on `storeId`, PrestaShop on the shop ID), so hosts register their own tenant-aware
handler for it under `CancelOrderHandler::OPERATION_TYPE`.

---

## Wiring the queue via `OutboundRequestQueueFactory`

`OutboundRequestQueueFactory` assembles a fully-wired queue with the default ComfinoPay error classifier and the pre-registered `cancel_order` handler.
...
In a multi-tenant host, implement `TenantAwareRetryableOperationHandlerInterface` instead, so the call is made with the
originating merchant's credentials rather than the cron scope's — see
[Per-tenant credentials on the drain](#per-tenant-credentials-on-the-drain).
...
| `TenantAwareRetryableOperationHandlerInterface` | Handler that is told which merchant it is delivering for |
