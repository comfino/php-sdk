...
final class ApiClientFactory
{
    ...
    /**
     * Creates a PSR-18-based API client pre-configured from a PlatformInfoInterface instance.
     *
     * Builds the User-Agent string from platform metadata, sets the API language, and enables sandbox mode when
     * requested.
     *
     * @param PlatformInfoInterface $platformInfo Platform metadata provider
     * @param string|null $apiKey Unique authentication key required for access to the Comfino API
    ...
    public function createClientFromPlatformInfo(
        ...
        $client = $this->createClient(
            ...
            sprintf(
                ...
                $platformInfo->getCode(),
                $platformInfo->getPluginVersion(),
                $platformInfo->getName(),
                $platformInfo->getVersion(),
                $platformInfo->getPhpVersion(),
                $platformInfo->getDomain()
            ),
