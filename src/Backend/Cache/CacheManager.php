 ...
 *
 * Call CacheManager::init() once during bootstrap with any PSR-6 pool implementation (e.g., FilesystemCachePool,
...
final class CacheManager
{
    ...
    {
        try {
            $item = self::getPool()->getItem(self::scopedKey($key));
    ...
    public static function set(string $key, mixed $value, int $ttl = 0, ?array $tags = null): void
    {
        ...
        for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
            try {
                $item = self::getPool()->getItem(self::scopedKey($key))->set($value);
    ...
    {
        return self::$scope === '' ? $key : preg_replace('/[^A-Za-z0-9_.]/', '_', self::$scope) . '.' . $key;
    }
}
