 ...
 * learns independently that ComfinoPay is down, so the outage costs one full timeout per worker per key instead of one.
 *
 ...
 * visible is the half-open probe: without a swap, the claim is a blind write, so *every* worker whose read interleaves
 * probes, not just one of them. That herd scales with the replica count — a fleet of twenty workers arriving together
 * on a half-open key sends twenty probes at a host that is, by assumption, still unhealthy. The lost increment really
 * is bounded (it delays opening by one call and no more); the probe herd is bounded only by how many workers are
 * running, so size it against the fleet before accepting it. Both are why this store is offered where an inexact
 * *rate limiter* store would not be (see {@see Psr6TokenBucketStore}). For a fleet where the herd matters, implement
