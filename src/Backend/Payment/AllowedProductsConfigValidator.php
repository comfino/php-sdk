    ...
    public function validateAndNormalize(string $rawJson): array
    {
        ...
        foreach ($decoded as $index => $entry) {
            ...
            if ($minTerm !== null && $maxTerm !== null && $minTerm > $maxTerm) {
                return self::failure(self::ERROR_MIN_TERM_GREATER_THAN_MAX_TERM, [$position, $type, $minTerm, $maxTerm]);
            }
        ...
        return self::success($normalized === [] ? '' : $this->serializer->serialize($normalized, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
    }

    /**
     * @param ErrorEntry|null $error Set to the failure descriptor on invalid input
     */
    private static function parseOptionalPositiveInt(mixed $value, int $position, string $fieldName, ?array &$error): ?int
    {
        if ($value === null || $value === '') {
            return null;
        }
