 ...
 * cannot hammer a ComfinoPay API that is already struggling. The cooldown deadline is persisted via {@see CacheManager}
 ...
 * grid (e.g. every 5 minutes, on the 5-minute marks), with no per-install offset. Without jitter, a ComfinoPay API
 * outage affecting many shops at once would end with all of them retrying at exactly the same tick once it
 * recovers. To spread that out, the actual cooldown applied is $cooldownSeconds plus a random extra of up to half
 * that (via {@see JitterInterface}), drawn fresh on every cooldown activation — this never shortens the configured
 * cooldown, only extends it unpredictably, so different installs (and different outages on the same install) end
 * up retrying at different offsets from the base grid instead of all landing on the same tick.
 *
 ...
 * whose ComfinoPay account is suspended must not hold every other merchant's drain behind its cooldown. Pass
...
final class OutboundRequestQueueProcessor
{
    ...
    private function beginCooldown(): void
    {
        try {
            ...
            $this->cache()->set(self::COOLDOWN_CACHE_KEY, $this->clock->now() + $jitteredCooldown, $jitteredCooldown, [self::CACHE_TAG]);
        } catch (Throwable) {
