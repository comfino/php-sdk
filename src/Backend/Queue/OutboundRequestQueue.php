...
/**
 * Durable queue for idempotent outbound ComfinoPay API calls, drained fairly across tenants.
 *
 * Two entry points:
 ...
 * one side only, or whose ComfinoPay account was suspended, failed on every attempt and stalled *every* other
 * merchant's cancellations behind it — indefinitely, because the stalled item stayed at the front. Three changes
 * remove that:
 *
...
final class OutboundRequestQueue
{
    ...
    public function submit(string $operationType, array $payload, ?string $tenantKey = null): SubmitResult
    {
        ...
        try {
            ...
            return match ($this->classifier->classify($operationType, $error)) {
                ...
                QueueErrorDisposition::DropPermanent => $this->onSubmitDropped($operationType, $payload, $error, $tenantKey),
                ...
                QueueErrorDisposition::Retry, QueueErrorDisposition::PauseTenant => $this->onSubmitDeferred($operationType, $payload, $error, $tenantKey),
            };
        }
    }
     ...
     * @param array<string, scalar> $payload
     */
    private function onSubmitDropped(string $operationType, array $payload, Throwable $error, ?string $tenantKey): SubmitResult
    {
        $this->log('error', '[REQUEST_QUEUE] submit dropped (permanent)', $operationType, $payload, $error, $tenantKey);

        $this->deadLetterReporter?->report(QueuedRequest::create($operationType, $payload, $this->clock->now(), $tenantKey), $error);
     ...
     * @param array<string, scalar> $payload
     */
    private function onSubmitDeferred(string $operationType, array $payload, Throwable $error, ?string $tenantKey): SubmitResult
    {
        $now = $this->clock->now();
    ...
    /**
     * Runs a handler, telling it which merchant the request belongs to when it can make use of that.
     *
     * The tenant key is the queue's answer to a defect rather than a feature: the payload alone leaves the API key to
     * be resolved from the ambient configuration scope, which on the drain path is the platform's cron scope and not
     * the merchant's - see {@see TenantAwareRetryableOperationHandlerInterface}. Handlers that do not implement that
     * interface keep the ambient behavior, so a single-tenant integration is unaffected.
     *
     * @param RetryableOperationHandlerInterface $handler Handler registered for the operation type
     * @param array<string, scalar> $payload Operation arguments
     * @param string|null $tenantKey Merchant the request belongs to
     *
     * @throws Throwable On any delivery failure (classified by the caller).
     */
