...
/**
 * Stateless helpers for IP address checks: CIDR membership, local/private detection, and ComfinoPay-server matching.
 *
 ...
 *
 * Example — verify that a webhook request really came from the ComfinoPay server:
 *
...
final class IpUtils
{
    /** Production IPv4 address of the ComfinoPay notification server. */
    public const COMFINO_SERVER_IP = '94.152.189.231';

    /**
     * IPv4 addresses the ComfinoPay **sandbox** notification server calls from.
     *
     * Originally recorded from integration testing rather than from a published specification (they are the egress
     * addresses of the sandbox environment's hosting), and **confirmed by the ComfinoPay API team on 2026-08-25**
     * as the addresses the sandbox notifies from. A host may enforce against this list rather than only reporting
     * on it.
     *
     * @var string[]
     */
    public const COMFINO_SANDBOX_SERVER_IPS = [
        '35.187.24.192',
        '35.205.155.59',
        '35.240.21.61',
        '34.77.240.220',
    ...
    /**
     * Returns true if $ip matches the ComfinoPay production notification server address.
     *
    ...
    /**
     * Returns true if $ip matches one of the ComfinoPay sandbox notification server addresses.
     *
     * @see COMFINO_SANDBOX_SERVER_IPS for where these addresses come from.
     */
    public static function isComfinoSandboxServerIp(string $ip): bool
    {
        return in_array($ip, self::COMFINO_SANDBOX_SERVER_IPS, true);
    }
