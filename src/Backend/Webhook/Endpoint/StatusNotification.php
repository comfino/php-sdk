...
/**
 * The `status` webhook: ComfinoPay tells the shop that a loan changed state.
 *
...
class StatusNotification extends WebhookEndpoint implements TenantAwareWebhookEndpointInterface
{
     ...
     */
    private function apply(ServerRequestInterface $serverRequest, ?string $endpointName, StatusManager|StatusAdapterInterface $target): void
    {
    ...
    private function validatedPayload(ServerRequestInterface $serverRequest, ?string $endpointName): ?array
    {
        ...
        if (!isset($requestPayload['status'])) {
            throw new InvalidRequest((string) $serverRequest->getUri(), $serverRequest->getBody()->getContents(), 'Status must be set.');
        }
        ...
        if (!isset($requestPayload['externalId'])) {
            throw new InvalidRequest((string) $serverRequest->getUri(), $serverRequest->getBody()->getContents(), 'External ID must be set.');
        }
