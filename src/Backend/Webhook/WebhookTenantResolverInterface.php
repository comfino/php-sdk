 ...
 * Comfino signature authorizes a request for *every* merchant. The request really is from Comfino, so verification
 * passes; it is then handled as whichever tenant the URL named. Resolving the tenant first and verifying against that
 * tenant's key only makes the confusion impossible.
 *
...
interface WebhookTenantResolverInterface
{
     ...
     * hand to Comfino, and there is no inbound request to read the tenant from.
     *
