...
final class WebhookManager
{
        ...
        if ($serverRequest === null) {
            try {
                ...
                return $this->getPreparedResponse(
                    $this->responseFactory->createResponse(self::HTTP_PAYLOAD_TOO_LARGE, $e->getMessage())
                );
            }
    ...
    private function routeRequest(
        ...
        foreach ($this->registeredEndpoints as $endpoint) {
            if (
                (string) $serverRequest->getUri() === $endpoint->getEndpointUrl() &&
                in_array($requestMethod, $endpoint->getMethods(), true)
            ) {
                return $endpoint;
            }
        }
     ...
     * merchant, and without the tenant one merchant's burst would throttle every other merchant's notifications. The
     * caller IP is the *resolved* one — see {@see ClientIpResolver} and the note in {@see processRequest()}.
     *
    ...
    private function enforceRateLimit(string $endpointName, VerifiedWebhookRequest $verified): ?ResponseInterface
    {
        ...
        $verdict = $this->limiter->consume(
            new RateLimitKey($endpointName, $verified->clientIp ?? '', $verified->tenantKey()),
