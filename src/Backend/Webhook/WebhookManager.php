 ...
 * - Automatic HTTP method-based response status codes (200/201/204/400/401/403/404/429).
 ...
 *
 * **On tenant resolution.** This class used to take a flat `string[] $apiKeys` and try each key until one matched. In a
 * host serving many merchants the natural wiring is to pass every merchant's key — and then any valid Comfino
 * signature authorizes a request for every merchant: the request really is from Comfino, so it verifies, and it is then
 * handled as whichever tenant the URL named. The library's default integration was the insecure one, and only hosts
 * that noticed on their own escaped it. A {@see WebhookTenantResolverInterface} makes the safe shape the default:
 * resolve the tenant from the request, then verify against that tenant's key alone. Single-shop plugins pass
 * {@see StaticApiKeyResolver}, or keep passing an array — the constructor wraps it in one — and see no change.
 */
final class WebhookManager
{
     ...
     *
     * Signs with the resolved tenant's most current key. In a multi-tenant host the tenant must be named: there is no
     * inbound request to read it from here, and signing with an arbitrary merchant's key would produce a value Comfino
     * attributes to the wrong shop.
     *
    ...
    public function getCrSignature(string $requestData, ?string $tenantKey = null): string
    {
        ...
        if ($tenant === null) {
            throw new AuthorizationError(
                sprintf(
                    'Unable to calculate CR-Signature: no API key configured for tenant "%s".',
                    $tenantKey ?? '(default)'
                )
            );
        }
    ...
    public function processRequest(
        ...
        try {
            ...
            $verified = $this->verifyRequest($serverRequest);
        ...
        if (($endpointName !== null) && ($endpoint = $this->getEndpointByName($endpointName)) !== null) {
            if (($rejection = $this->enforceRateLimit($endpoint, $clientIp, $verified)) !== null) {
                return $rejection;
            }
     ...
     * Comfino signature for any merchant, since the signature proves who *sent* the request, not whose shop it is
     ...
     */
    protected function verifyRequest(ServerRequestInterface $request): VerifiedWebhookRequest
    {
        ...
        if ($this->replay?->isDuplicate($crSignature, $tenant->tenantKey)) {
            // If the request is a duplicate, throw an exception.
            throw new AccessDenied('Duplicate webhook request.');
        }

        return new VerifiedWebhookRequest($tenant, $crSignature, (string) $this->calculatedCrSignature);
    }
    ...
    {
        $this->replay?->markProcessed(
            $verified->receivedSignature,
            $verified->tenantKey(),
     ...
     * Comfino webhooks arrive from Comfino's own infrastructure, so the caller IP identifies the sender rather than the
     * merchant, and without the tenant one merchant's burst would throttle every other merchant's notifications.
     *
     ...
     *
     * @param WebhookEndpointInterface $endpoint The endpoint the request routes to
     * @param string $clientIp Caller IP address
     * @param VerifiedWebhookRequest $verified The verification outcome, carrying the tenant
     *
    ...
    private function enforceRateLimit(
        ...
        $verdict = $this->limiter->consume(
            new RateLimitKey($endpoint->getName(), $clientIp, $verified->tenantKey()),
