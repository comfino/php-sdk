 ...
 *
 * Example — ComfinoPay production only:
 *
...
final class IpWhitelist implements IpWhitelistInterface
{
    ...
    /**
     * Creates a whitelist pre-configured for the ComfinoPay notification servers.
     *
    ...
    public static function forComfino(
        ...
        return new self($allowedIps, allowLocalAddresses: $allowLocalAddresses, enforce: $enforce, logger: $logger);
    }
    ...
    private function reject(?string $clientIp): bool
    {
        $this->logger?->warning(
            $this->enforce
                ? 'ComfinoPay webhook rejected: source address is not on the allow-list.'
                : 'ComfinoPay webhook from an address that is not on the allow-list; enforcement is off, so it passed.',
