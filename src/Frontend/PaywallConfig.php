...
/**
 * Represents the configuration required to initialize the ComfinoPay Paywall SDK.
 */
final class PaywallConfig
{
     ...
     * @param string $loggingToken Frontend error-reporting auth token (base64-encoded); empty string when unavailable
     * @param string $trackId Backend correlation ID from the PHP API client (AbstractClient::getTrackId()); empty
     *        string when unavailable. Forwarded to the frontend SDK as the Comfino-Track-Id HTTP header on error
     *        reports so browser-side errors can be correlated with backend API calls.
     */
    public function __construct(
        ...
        public readonly string $loggingToken = '',
    ...
    public function getAsArray(): array
    {
        return [
            ...
            'loggingToken' => $this->loggingToken,
