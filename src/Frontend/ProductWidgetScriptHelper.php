...
/**
 * Renders the default ComfinoPay product-page widget initialization: a JSON config block plus the CDN-hosted product
 ...
 * ComfinoPay-maintained shop plugins pass their platform to {@see getScriptUrl()} to load their dedicated per-platform
...
class ProductWidgetScriptHelper
{
     ...
     * @param string|null $platform ComfinoPay platform id for the dedicated per-platform script (e.g. 'prestashop',
     ...
     */
    public static function getScriptUrl(bool $sandboxMode, ?string $platform = null, int $version = self::DEFAULT_SCRIPT_VERSION): string
    {
    ...
    public static function buildConfig(array $values): array
    {
        ...
        return array_filter($values, static fn ($value, $key) => isset($allowed[$key]) && $value !== null, ARRAY_FILTER_USE_BOTH);
    }
    ...
    public static function renderScript(array $config, string $url, ?SerializerInterface $serializer = null): string
    {
        ...
        try {
            $json = $serializer->serialize($config, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES);
