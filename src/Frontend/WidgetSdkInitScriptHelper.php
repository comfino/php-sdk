...
/**
 * Helper class for rendering the ComfinoPay widget initialization script for the ComfinoPay Web SDK.
 *
 * Targets comfino-sdk.min.js and initializes the widget via the imported SDK module's ComfinoSDK.getInstance(),
 * sdk.init(), and sdk.bootstrapWidget() (the SDK exposes no window.ComfinoPay global). Use WidgetInitScriptHelper
 * (widget-frontend.min.js facade) for integrations that require the legacy ComfinoWidgetFrontend.init() interface.
 *
 ...
 *  - DOM params (WIDGET_PRICE_SELECTOR, WIDGET_PRICE_OBSERVER_SELECTOR, WIDGET_PRICE_OBSERVER_LEVEL, EMBED_METHOD) are
 *    kept in PARAMS — the SDK's bootstrapWidget() consumes them for price detection, hardened dynamic-price tracking,
 *    and banner insertion. The plugin template no longer needs to place the container element itself.
 */
class WidgetSdkInitScriptHelper
{
     ...
     * DOM params — consumed by bootstrapWidget's price observer and banner-insertion logic.
     */
     ...
     * SHOP_ENVIRONMENT — JSON object with platform, version, domain, plugin version, etc.
     */
    ...
    /**
     * Renders the ComfinoPay widget initialization script.
     *
     * @param string $widgetInitCode The ComfinoPay widget initialization JavaScript code
     * @param array<string, mixed> $widgetInitParams The ComfinoPay widget initialization parameters
     * @param array<string, mixed> $widgetInitVariables The ComfinoPay widget initialization variables
     * @param SerializerInterface|null $serializer Optional JSON serializer; if null, a default Json serializer is used
     *
     * @return string The rendered ComfinoPay widget initialization script
     *
    ...
    public static function renderWidgetInitScript(
        ...
        $serializeValue = static function ($varValue) use ($serializer): string {
            ...
            /* The same defensive flags are applied to arrays as to scalars: the helper output is embedded directly into
               ...
               script context. */
            return $serializer->serialize($varValue, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES);
        };
        ...
        return str_replace(
            ...
            array_merge(
                array_map(
                    $serializeValue,
                    array_merge(array_merge($widgetInitParamsAssocKeys, $widgetInitParams), array_values($widgetInitVariables))
                ),
                ['true', 'false', 'null']
            ),
            $widgetInitCode
