...
{
    /** Path, relative to the ComfinoPay CDN origin, of the shared checkout payment-method-item stylesheet. */
    private const CHECKOUT_LOADER_CSS_PATH = '/css/comfino-checkout.css';

    /** Path template, relative to the ComfinoPay CDN origin, of a platform's checkout glue bundle (%s = platform slug). */
    private const CHECKOUT_SCRIPT_PATH_TEMPLATE = '/checkout/v1/comfino-%s.min.js';

    /** Path template, relative to the ComfinoPay CDN origin, of a platform's product-page widget bundle (%s = platform slug). */
     ...
     * payment-method-item visual layer, e.g., the loading-placeholder spinner. The URL is derived from the
     * ComfinoPay web SDK script URL's origin, so it always points at the same CDN as the SDK (production / sandbox /
     * dev) without each shop module hardcoding or re-deriving the host. Returns null when the SDK script URL is
     * empty or malformed.
     *
     * @param string $sdkScriptUrl The ComfinoPay web SDK script URL (as returned by the shop's config manager)
     *
    ...
    /**
     * Generates a logo authentication hash for the ComfinoPay payment gateway.
     *
    ...
    {
        return rawurlencode(base64_encode(self::getLogoAuthKey($platformCode, $platformVersion, $pluginVersion, $buildTimestamp)));
    }

    /**
     * Generates a paywall logo authentication hash for the ComfinoPay payment gateway.
     *
     * @param string $platformCode The shop platform code
     * @param string $platformVersion The shop platform version
     * @param string $pluginVersion The shop plugin version
    ...
    /**
     * Generates a logo authentication key for the ComfinoPay payment gateway.
     *
     * @param string $platformCode The shop platform code
     * @param string $platformVersion The shop platform version
     * @param string $pluginVersion The shop plugin version
    ...
    /**
     * Renders the ComfinoPay payment gateway logo.
     *
     * @param string $apiBaseUrl The ComfinoPay payment gateway API base URL
     * @param string $platformCode The shop platform code
     * @param string $platformVersion The shop platform version
     * @param string $pluginVersion The shop plugin version
     * @param int $buildTimestamp The shop plugin build timestamp
     ...
     *
     * @return string The rendered ComfinoPay payment gateway logo
     */
    ...
    /**
     * Renders the ComfinoPay payment gateway paywall logo.
     *
     * @param string $apiBaseUrl The ComfinoPay payment gateway API base URL
     * @param string $apiKey The shop plugin API key
     * @param string $widgetKey The shop widget key
     ...
     * @param string $alt The logo image alt text
     *
     * @return string The HTML image tag for the ComfinoPay paywall logo
     */
    ...
    /**
     * Extracts the scheme://host[:port] origin from the ComfinoPay web SDK script URL, so CDN-served sibling assets
     * (stylesheet, checkout glue) can be addressed on the same host as the SDK. Returns null when the URL is empty or
     * lacks a scheme/host.
     *
     * @param string $sdkScriptUrl The ComfinoPay web SDK script URL
     *
    ...
    /**
     * Renders the ComfinoPay payment gateway logo image.
     *
     ...
     *
     * @return string The HTML image tag for the ComfinoPay logo
     */
