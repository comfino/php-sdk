...
/**
 * Cross-platform builder for the ComfinoPay web SDK (comfino-sdk.min.js) and backend API URLs.
 *
 ...
 *     allow-list (ComfinoPay domains over HTTPS, private/loopback IPs, single-label hostnames) — so a tampered env var
...
final class SdkUrlBuilder
{
    ...
    /**
     * Builds the ComfinoPay web SDK (ESM) script URL for the given environment and SDK version. The bundle is always an
     ...
     */
    public static function getSdkScriptUrl(bool $sandboxMode, bool $devOverridesEnabled = false, int $version = self::DEFAULT_SDK_VERSION): string
    {
    ...
    /**
     * Builds the ComfinoPay product-page widget script URL (the classic-IIFE glue that reads the
     ...
     * ComfinoPay-maintained plugin's dedicated per-platform script.
     *
     * @param bool $sandboxMode Whether the shop runs in sandbox mode
    ...
    public static function getProductWidgetScriptUrl(
        ...
        return self::rehost(ProductWidgetScriptHelper::getScriptUrl($sandboxMode, $platform, $version), $devOverridesEnabled);
    }

    /**
     * Builds the CDN URL of the default ComfinoPay brand logo (the payment-tile placeholder shown before the SDK
     * renderer swaps it for the auth-gated API logo).
     *
     * @param bool $sandboxMode Whether the shop runs in sandbox mode
     * @param bool $devOverridesEnabled The plugin's per-shop dev-override opt-in
     *
    ...
    {
        if (self::devEnvActive($devOverridesEnabled) && ($baseUrl = self::safeOverride('COMFINO_DEV_SDK_CDN_BASE_URL')) !== null) {
