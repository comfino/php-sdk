...
class Cart
{
    ...
    {
        return array_reduce($this->cartItems, static fn (int $carry, CartItemInterface $cartItem): int => $carry + $cartItem->getQuantity(), 0);
    }
     ...
     *
     * This method is intended for plugin consumers that need to pass cart data to the frontend layer (e.g.,
     * ComfinoPay web frontend SDK). It includes product-level categoryIds, which are required for frontend category
     * filtering, and preserves null values by default so that JavaScript can distinguish missing fields.
     *
     * Do NOT confuse this with CartTrait::getCartAsArray() from the php-api-client library, which serializes a
     * CartInterface to the ComfinoPay API wire format (strips nulls, adds discount/correction items when cart item
     * totals diverge from the order total, and omits categoryIds).
     *
