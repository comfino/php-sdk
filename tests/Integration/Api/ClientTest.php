...
final class ClientTest extends TestCase
{
    ...
    {
        $this->assertNotEmpty($this->client->getFinancialProducts(new LoanQueryCriteria(loanAmount: 150000, loanTerm: 12))->financialProducts);
    }
    ...
    public function testGetFinancialProductDetails(): void
    {
        ...
        $detailCriteria = new LoanQueryCriteria(loanAmount: 150000, loanTerm: $firstProduct->loanTerm, loanType: $firstProduct->type);

        $cart = new Cart(
            items: [
                new CartItem(product: new Product(name: 'Test Product', price: 150000, id: 'PROD-001', category: 'Electronics'), quantity: 1),
            ],
            totalAmount: 150000
        );
    ...
    public function testCreateOrder(): void
    {
        ...
        /* The order is deliberately not read back: php-api-client 3.0 removed getOrder(), because polling the API for
           an order's state is the wrong shape - the status webhook pushes every transition, and the poll only ever
           raced it. Order status reconciliation is covered by the webhook endpoint tests instead. */
    }
    ...
    {
        $product = new Product(name: 'Test Product', price: $amount, id: 'PROD-001', category: 'Electronics');
