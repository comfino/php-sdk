...
final class CacheManagerTest extends TestCase
{
    ...
    public function testGetPrefixesKeyWithScope(): void
    {
        $item = $this->mockItem('hello', true);
        $pool = $this->mockPool();
        $pool->method('getItem')->with('42.my-key')->willReturn($item);

        CacheManager::init($pool, '42');

        $this->assertSame('hello', CacheManager::get('my-key'));
    }

    public function testSetPrefixesKeyWithScope(): void
    {
        $item = $this->mockItem();
        $pool = $this->mockPool();
        $pool->method('getItem')->with('42.key')->willReturn($item);
        $pool->expects($this->once())->method('save')->with($item);
    ...
    public function testScopeSanitizesUnsafeCharacters(): void
    {
        $item = $this->mockItem('hello', true);
        $pool = $this->mockPool();
        $pool->method('getItem')->with('store_2.my-key')->willReturn($item);

        CacheManager::init($pool, 'store/2');

        $this->assertSame('hello', CacheManager::get('my-key'));
    }

    public function testDifferentScopesDoNotShareCachedValues(): void
    {
        $itemA = $this->mockItem('value-a', true);
        $itemB = $this->mockItem();
        ...
        CacheManager::init($pool, '1');

        $this->assertSame('value-a', CacheManager::get('shared-key'));

        CacheManager::init($pool, '2');

        $this->assertNull(CacheManager::get('shared-key'));
    }
