 ...
 *
 * With the process-local defaults each worker learns independently that Comfino is down and each worker gets its own
...
final class Psr6StoresTest extends TestCase
{
    ...
    public function testAnAbsentBucketReadsAsNullSoTheKeyStartsFull(): void
    {
        $limiter = new TokenBucketRateLimiter(
            2,
            1.0,
            new Psr6TokenBucketStore(new ArrayCachePool()),
            new FrozenClock(1000.0)
        );
    ...
    public function testTwoWorkersSharingThePoolShareTheBudget(): void
    {
        ...
        self::assertFalse(
            $workerA->reserve('shop-a')->accepted,
            'Sequentially the shared budget holds; it is only a genuine race the store cannot resolve.'
        );
    }
    ...
    public function testAKeyKeepsAReadableFragmentSoACacheRowCanBeRecognized(): void
    {
        self::assertStringContainsString(
            'shop_a',
            Psr6StoreKey::build('comfino_cb', 'shop-a|api-ecommerce.comfino.pl')
        );
    }
