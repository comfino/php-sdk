...
final class DebugLoggerTest extends TestCase
{
    ...
    public function testLogEventRendersEventTypeLabelAndCapturesCaller(): void
    {
        ...
        $psr3Logger->expects($this->once())
            ...
            ->with(
                ...
                $this->callback(static fn (array $params): bool => $params['key'] === 'value' && isset($params['_caller']))
            );
    ...
    public function testLogEventWithoutEventTypeKeepsMessageClean(): void
    {
        ...
        $psr3Logger->expects($this->once())
            ->method('debug')
            ->with($this->equalTo('Simple message'), $this->callback(static fn (array $params): bool => isset($params['_caller'])));
    ...
    public function testLogEventConditionalLogsWhenDebugAndServiceModeActive(): void
    {
        ...
        $psr3Logger->expects($this->once())
            ->method('debug')
            ->with($this->equalTo('[API_TIMEOUT] Timed out'), $this->callback(static fn (array $params): bool => isset($params['_caller'])));
    ...
    {
        $this->assertEmpty(DebugLogger::getInstance($this->tempLogFile)->getDebugLog(10), 'Should return empty string when no log file exists.');
    }
    ...
    {
        mkdir($this->tempLogDir, 0755, true);
        file_put_contents($this->tempLogFile, "[LOG_1]: First log entry\n[LOG_2]: Second log entry\n[LOG_3]: Third log entry\n");
