...
final class ApiTransientErrorClassifierTest extends TestCase
{
     ...
     */
    public function testClassifiesHttpStatusCodes(string $operationType, int $statusCode, QueueErrorDisposition $expected): void
    {
        self::assertSame($expected, $this->classifier->classify($operationType, new HttpErrorStub($statusCode)));
    }
    ...
    public static function statusCodeProvider(): array
    {
        return [
            ...
            '401 → pause tenant' => ['cancel_order', 401, QueueErrorDisposition::PauseTenant],
            '403 → pause tenant' => ['cancel_order', 403, QueueErrorDisposition::PauseTenant],
    ...
    public function testNetworkTimeoutIsRetryable(): void
    {
        ...
        self::assertSame(QueueErrorDisposition::Retry, $this->classifier->classify('cancel_order', $curlTimeout));
    }

    public function testUnknownErrorIsRetriedConservatively(): void
    {
        self::assertSame(QueueErrorDisposition::Retry, $this->classifier->classify('cancel_order', new RuntimeException('unexpected')));
    }
    ...
    public function testAbsorbNotFoundOperationsAreConfigurable(): void
    {
        ...
        self::assertSame(QueueErrorDisposition::TreatAsSuccess, $classifier->classify('refund_order', new HttpErrorStub(404)));
        self::assertSame(QueueErrorDisposition::DropPermanent, $classifier->classify('cancel_order', new HttpErrorStub(404)));
    }
}
