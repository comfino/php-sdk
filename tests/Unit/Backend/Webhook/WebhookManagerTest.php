...
final class WebhookManagerTest extends TestCase
{
    ...
    /**
     * @param mixed[]|null $apiKeys Deliberately loose: host platforms may pass null or empty candidate keys
     */
    ...
    private function createMockRequest(
        ...
        if (isset($headers['CR-Signature'])) {
            ...
            $request->method('getHeader')->willReturnCallback(fn ($h) => $h === 'CR-Signature' ? [$headers['CR-Signature']] : []);
        } elseif (isset($headers['X-CR-Signature'])) {
            $request->method('hasHeader')->willReturnCallback(fn ($h) => $h === 'X-CR-Signature');
            $request->method('getHeader')->willReturnCallback(fn ($h) => $h === 'X-CR-Signature' ? [$headers['X-CR-Signature']] : []);
        } else {
    ...
    public function testProcessRequestSupportsXCrSignatureHeader(): void
    {
        ...
        $request = $this->createMockRequest('GET', ['vkey' => $validationKey], ['X-CR-Signature' => $expectedSignature]);
        $response = $this->createMockResponse();
    ...
    public function testProcessRequestRejectsForgedSignatureWhenOneApiKeyIsNull(): void
    {
        $requestBody = '{"externalId":"1042","status":"ACCEPTED"}';
        $request = $this->createMockRequest('PUT', [], ['CR-Signature' => hash('sha3-256', $requestBody)], $requestBody);
        $response = $this->createMockResponse();
        ...
        // A null candidate key must be skipped, not passed to the verifier, where it would raise a TypeError.
        $manager = $this->createManager(['real_production_key', null]);

        $this->assertSame($response, $manager->processRequest(null, $request));
    }

    public function testProcessRequestRejectsSignatureCalculatedFromAnEmptyApiKey(): void
    {
        $requestBody = '{"externalId":"1042","status":"ACCEPTED"}';
        $request = $this->createMockRequest('PUT', [], ['CR-Signature' => hash('sha3-256', $requestBody)], $requestBody);
        $response = $this->createMockResponse();
        ...
        /* An empty candidate key would make the expected signature a plain hash of the request body, which any
           caller can compute, so it must never take part in verification. */
        $manager = $this->createManager(['real_production_key', '']);

        $this->assertSame($response, $manager->processRequest(null, $request));
    }

    public function testProcessRequestAcceptsValidSignatureWhenOtherApiKeysAreUnusable(): void
    {
        $requestBody = '{"externalId":"1042","status":"ACCEPTED"}';
        $expectedSignature = hash('sha3-256', 'real_production_key' . $requestBody);
    ...
    public function testProcessRequestReturnsUnauthorizedWhenNoApiKeyIsConfigured(): void
    {
        $validationKey = 'test_vkey';
        $request = $this->createMockRequest('GET', ['vkey' => $validationKey], ['CR-Signature' => hash('sha3-256', $validationKey)]);
        $response = $this->createMockResponse();
        ...
        $this->assertSame($response, $this->createManager([null, null])->processRequest(null, $request));
    }

    public function testGetCrSignatureThrowsWhenNoApiKeyIsConfigured(): void
    {
        $this->expectException(AuthorizationError::class);
        $this->expectExceptionMessage('Unable to calculate CR-Signature: no API key configured for tenant "(default)".');

        $this->createManager([null, ''])->getCrSignature('test_vkey');
    }
    ...
    public function testReplayProtectionBlocksDuplicateRequest(): void
    {
        ...
        $this->responseFactory->method('createResponse')->with(403, 'Duplicate webhook request.')->willReturn($response);
    ...
    public function testIpWhitelistBlocksRequestAndReturns403(): void
    {
        $ipWhitelist = $this->createMock(IpWhitelistInterface::class);
        $ipWhitelist->expects($this->once())->method('isAllowed')->willReturn(false);

        $endpoint = $this->createMockEndpoint('ep', '/api/ep', ['GET']);
        $endpoint->expects($this->never())->method('processRequest');

        $manager = $this->createManager(ipWhitelist: $ipWhitelist);
        $manager->registerEndpoint($endpoint);

        // No signature needed — IP check runs before signature verification.
        $request  = $this->createMockRequest('GET');
        $response = $this->createMockResponse();

        $this->responseFactory->method('createResponse')->with(403, 'Access forbidden.')->willReturn($response);

        $this->assertSame($response, $manager->processRequest('ep', $request));
    }

    public function testIpWhitelistAllowsRequestAndContinuesToSignatureCheck(): void
    {
        $validationKey = 'test_vkey';
        $signature     = hash('sha3-256', $this->apiKeys[0] . $validationKey);

        $ipWhitelist = $this->createMock(IpWhitelistInterface::class);
        $ipWhitelist->expects($this->once())->method('isAllowed')->willReturn(true);
