...
final class WebhookTenantResolutionTest extends TestCase
{
    ...
    /**
     * The rate limit is counted against the resolved tenant. All Comfino webhooks arrive from Comfino's own
    ...
    public function testGetCrSignatureSignsWithTheNamedTenantsKey(): void
    {
        ...
        self::assertSame(
            hash('sha3-256', self::SHOP_A_KEY . 'vkey_value'),
            $manager->getCrSignature('vkey_value', 'shop_a')
        );
        self::assertSame(
            hash('sha3-256', self::SHOP_B_KEY . 'vkey_value'),
            $manager->getCrSignature('vkey_value', 'shop_b')
        );
    }
    ...
    public function testAnArrayOfKeysStillWorksAndResolvesStatically(): void
    {
        ...
        self::assertSame(
            hash('sha3-256', self::SHOP_A_KEY . 'vkey_value'),
            $manager->getCrSignature('vkey_value')
        );
    }
    ...
    private function createRequest(
        ...
        $request->method('getAttribute')->willReturnCallback(
            static fn (string $name) => $name === 'installation_id' ? $installationId : null
        );
        $request->method('hasHeader')->willReturnCallback(static fn ($h): bool => $h === 'CR-Signature');
        $request->method('getHeader')->willReturnCallback(
            static fn ($h): array => $h === 'CR-Signature' ? [$signature] : []
        );
