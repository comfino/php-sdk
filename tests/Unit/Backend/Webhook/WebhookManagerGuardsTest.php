...
final class WebhookManagerGuardsTest extends TestCase
{
    ...
    public function testARequestFallingThroughSeveralEndpointsIsChargedOnce(): void
    {
        ...
        $manager->registerEndpoint(new RecordingEndpoint('second', 'https://shop.example/hooks/second', accepts: false));
        ...
        self::assertCount(1, $limiter->consumed, 'One request must cost one token, whatever the endpoint list looks like.');
    ...
    public function testTheRateLimitKeyCarriesTheResolvedCallerAddress(): void
    {
        ...
        $manager->processRequest('status', $this->request(headers: ['X-Forwarded-For' => '203.0.113.7'], serverParams: ['REMOTE_ADDR' => '10.0.0.9']));
    ...
    public function testAResolverWithNoTrustedProxiesTakesRemoteAddrAsGiven(): void
    {
        ...
        $manager->processRequest('status', $this->request(headers: ['X-Forwarded-For' => '198.51.100.1'], serverParams: ['REMOTE_ADDR' => '203.0.113.7']));
    ...
    public function testTheAllowListAndTheLimiterJudgeTheSameAddress(): void
    {
        ...
        $manager = $this->manager(
            rateLimiter: $limiter,
            ipWhitelist: new IpWhitelist(['203.0.113.7'], allowLocalAddresses: false, ipResolver: $resolver),
            ipResolver: $resolver
        );
        $manager->registerEndpoint(new RecordingEndpoint('status', '/hooks/status'));
    ...
    {
        $replay = new RecordingReplayProtection();
        $manager = $this->manager(replayProtection: $replay, replayKeyExtractor: new HeaderReplayKeyExtractor());
        $manager->registerEndpoint(new RecordingEndpoint('status', '/hooks/status'));
    ...
    {
        $manager = $this->manager(replayProtection: new RecordingReplayProtection(), replayKeyExtractor: new HeaderReplayKeyExtractor());
        $manager->registerEndpoint(new RecordingEndpoint('status', '/hooks/status'));
