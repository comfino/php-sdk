...
final class ProductTypeFilterManagerTest extends TestCase
{
    ...
    /**
     * The filters registered here are a per-tenant fact - built from that merchant's configured cart-value limits and
     * excluded categories - so a process-wide singleton silently narrowed which financial products another merchant's
     * shoppers were offered.
     */
    public function testFiltersRegisteredForOneScopeDoNotApplyToAnother(): void
    {
        $shopA = ProductTypeFilterManager::getInstance('shop_a');
        $shopB = ProductTypeFilterManager::getInstance('shop_b');

        $this->assertNotSame($shopA, $shopB);

        $shopA->addFilter($this->createMock(ProductTypeFilterInterface::class));

        $this->assertTrue($shopA->filtersActive());
        $this->assertFalse($shopB->filtersActive());
    }

    public function testGetInstanceReturnsTheSameInstancePerScope(): void
    {
        $this->assertSame(ProductTypeFilterManager::getInstance('shop_a'), ProductTypeFilterManager::getInstance('shop_a'));
        $this->assertSame('shop_a', ProductTypeFilterManager::getInstance('shop_a')->getScope());
    }
    ...
    public function testResetDropsOnlyTheNamedScope(): void
    {
        ...
        ProductTypeFilterManager::reset('shop_a');

        $this->assertNotSame($shopA, ProductTypeFilterManager::getInstance('shop_a'));
        $this->assertSame($shopB, ProductTypeFilterManager::getInstance('shop_b'));
    }
    ...
    public function testGetAllowedProductTypesAppliesSingleFilter(): void
    {
        ...
        $this->assertSame(
            [LoanType::PAY_LATER],
            $manager->getAllowedProductTypes([LoanType::PAY_LATER, LoanType::INSTALLMENTS_ZERO_PERCENT], $this->makeCart())
        );
    }
    ...
    public function testGetAllowedProductTypesIntersectsMultipleFilters(): void
    {
        ...
        $this->assertSame(
            ...
            $manager->getAllowedProductTypes([LoanType::PAY_LATER, LoanType::INSTALLMENTS_ZERO_PERCENT, LoanType::CONVENIENT_INSTALLMENTS], $this->makeCart())
        );
    }
