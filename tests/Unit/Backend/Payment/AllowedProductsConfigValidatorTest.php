...
final class AllowedProductsConfigValidatorTest extends TestCase
{
    ...
    {
        $result = $this->validator->validateAndNormalize('[{"terms":[6,3,6,3],"maxTerm":6,"minTerm":1,"type":"PAY_LATER"}]');

        self::assertTrue($result['valid']);
        self::assertSame([], $result['errors']);

        // Canonical: keys ordered type/minTerm/maxTerm/terms, terms deduped preserving first-seen order.
        self::assertSame('[{"type":"PAY_LATER","minTerm":1,"maxTerm":6,"terms":[6,3]}]', $result['normalizedJson']);

        // The normalized value must be consumable by the SDK builder that reads it back.
        $dtos = AllowedProductsConfigBuilder::fromPersistedArray(json_decode($result['normalizedJson'], true, 512, JSON_THROW_ON_ERROR));

        self::assertNotNull($dtos);
        self::assertCount(1, $dtos);
        self::assertSame(
            $result['normalizedJson'],
            (string) json_encode(AllowedProductsConfigBuilder::toFrontendArray($dtos), JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
        );
    }
}
